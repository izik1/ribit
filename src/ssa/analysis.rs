...
mod test {
    ...
    fn dead_reads_give_valid_lifetimes() {
        ...
        let mut ctx = crate::ssa::lower::Context::new(0x10000);
        ...
        for instr in rest {
            match instr {
                BinaryInstruction::Big(it) => {
                    crate::ssa::lower::non_terminal(
                        &mut ctx,
                        crate::decode::instruction(*it).unwrap(),
                        4,
                    );
                }
                ...
                BinaryInstruction::Small(it) => {
                    crate::ssa::lower::non_terminal(
                        &mut ctx,
                        crate::decode::compressed::decode_instruction(*it).unwrap(),
                        2,
                    );
                }
            }
        ...
        let (mut instrs, _) = match last {
            BinaryInstruction::Big(it) => {
                crate::ssa::lower::terminal(ctx, crate::decode::instruction(*it).unwrap(), 4)
            }

            BinaryInstruction::Small(it) => crate::ssa::lower::terminal(
                ctx,
                crate::decode::compressed::decode_instruction(*it).unwrap(),
                2,
            ),
